...
/**
 * Feature Flag System — SELIDIKI Architecture v1.0
 *
 * Central configuration for all runtime feature flags.
 * ALL repository routing decisions MUST read from this module.
 *
 * Kill Switch: ENABLE_DATABASE_SWITCHING
 *   When FALSE → ALL reads/writes go to Firestore, regardless of other flags.
 *   This is the emergency brake for production deployments.
 */
...
// ── Legacy Provider Mapping (backward compat) ───────────────
if (!DATABASE_SWITCHING && LEGACY_PROVIDER !== 'FIRESTORE') {
  // Legacy provider present but switching disabled — override safely
}

function applyLegacyProvider() {
  if (!DATABASE_SWITCHING) return; // Kill switch: ignore legacy provider
  const mapping = {
    FIRESTORE:  {},
...
// ── Effective Flags (after applying legacy mapping) ─────────
...
const flags = {
  ...
  // Mode flags (all force-disabled when kill switch is off)
...
// ── Validation (must pass at startup) ───────────────────────
...
function startupSnapshot() {
  ...
  if (!DATABASE_SWITCHING) {
    console.log('⚠️  ENABLE_DATABASE_SWITCHING=false — Kill switch ENGAGED. All traffic → Firestore.');
  }
}
